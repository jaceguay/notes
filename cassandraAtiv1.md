...
```bash
CREATE MATERIALIZED VIEW usuarioReceptor_ASC AS
    SELECT * FROM menssagens_ASC
...
SELECT * FROM usuarioReceptor_ASC WHERE login = 'joao' ALLOW FILTERING;

 login_dest | login | data                            | conteudo            | email          | localizacao | nome          | origem | telefone       | tipo
------------+-------+---------------------------------+---------------------+----------------+-------------+---------------+--------+----------------+-------
      paulo |  joao | 2019-01-11 00:00:00.000000+0000 | Reunião às 14:00hrs | mjua@gmail.com |     Brusque | Joao da Silva |    Web | (47)93389-2227 | vídeo
      maria |  joao | 2019-11-19 00:00:00.000000+0000 |          Boa noite! | joao@gmail.com |      Gaspar |    Maria José |    APP | (47)93389-2227 | vídeo
       rita |  joao | 2019-12-11 00:00:00.000000+0000 |  Pagar conta de luz |   pr@gmail.com |      Itajaí |    Maria José |    Web | (47)93389-2227 | vídeo
...
```bash
SELECT * FROM menssagens_ASC WHERE login = 'maria' LIMIT 10;
...
```bash
CREATE MATERIALIZED VIEW usuarioReceptor_DESC AS
    SELECT * FROM menssagens_ASC
    WHERE login IS NOT NULL AND login_dest IS NOT NULL AND data IS NOT NULL
    PRIMARY KEY (data, login, login_dest)
    WITH CLUSTERING ORDER BY (data DESC);

SELECT * FROM usuarioReceptor_DESC WHERE login = 'joao' AND login_dest = 'maria' ALLOW FILTERING;

 data                            | login | login_dest | conteudo                  | email          | localizacao | nome       | origem | telefone       | tipo
---------------------------------+-------+------------+---------------------------+----------------+-------------+------------+--------+----------------+-------
 2019-02-20 00:00:00.000000+0000 |  joao |      maria |    Lavação carro 12:00hrs |   pr@gmail.com |     Brusque | Júlia José |    APP | (47)97789-8888 | vídeo
 2019-01-11 00:00:00.000000+0000 |  joao |      maria | Buscar filho no aeroporto |   pr@gmail.com |     Brusque | Júlia José |    Web | (47)99999-9999 | áudio
 2019-11-19 00:00:00.000000+0000 |  joao |      maria |                Boa noite! | joao@gmail.com |      Gaspar | Maria José |    APP | (47)93389-2227 | vídeo

(3 rows)
```
10. Relacione o nome de todos os usuários ordenados pela data de última mensagem transmitida pelo sistema.
```bash
SELECT distinct login, MAX(data) FROM menssagens_ASC;
SELECT distinct login FROM menssagens_ASC;
```
